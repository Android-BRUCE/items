...
<!--  搜索表单  -->
<form name='form3' action="${pageContext.request.contextPath}/Email/Email_dimquery.action" method='get'>
    ...
    <td background='skin/images/wbg.gif' align='center'>
      <table border='0' cellpadding='0' cellspacing='0'>
        ...
        <td width='110'>
            ...
            <option value="1">发送时间(升序)</option>
            <option value="2">发送时间(降序)</option>
      	</select>
        </td>
        <td>
...
<!--  内容列表   -->
		<form id="form2" name="form2" action="${pageContext.request.contextPath}/Email/Email_query.action" method="post">
			...
			<tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22" >
				...
				<td>${listEmail.receivename }</td>
				<td>${listEmail.email_senDate }</td>
				<td><a href="${pageContext.request.contextPath}/Email/Email_delete.action?email_id=${listEmail.email_id }" id="remove"  onclick="return deletes()">删除</a></td>
			</tr>
			</c:forEach>
			...
			<td height="28" colspan="12">
				...
				<a href="${pageContext.request.contextPath}/Email/Email_delete.action" class="coolbg">&nbsp;删除&nbsp;</a>
	...
	<input type="text" name="pageSize" style=" width: 25px; height: 22px" onblur="page();" value="${pageView.pageSize }" id="pageSize"/>条纪录
            ...
            <span style="font-size:12px">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
               <c:if test="${pageView.pageNum>1}">
                          <a href="${pageContext.request.contextPath}/Email/Email_query.action?pageNum=${pageView.pageNum-1}&pageSize=${pageView.pageSize }">上一页</a>
               </c:if>
               <c:forEach var="i" begin="1" end="${pageView.totalPages}" step="1">
               		<c:choose>
                      	<c:when test="${pageView.currentPage==i}">${i}&nbsp;&nbsp;</c:when>
                      	<c:otherwise><a href="${pageContext.request.contextPath}/Email/Email_query.action?pageNum=${i}&pageSize=${pageView.pageSize }">${i}</a>&nbsp;&nbsp;</c:otherwise>
                    </c:choose>
               </c:forEach>
               
               <c:if test="${pageView.pageNum<pageView.totalPages}">
                          <a href="${pageContext.request.contextPath}/Email/Email_query.action?pageNum=${pageView.pageNum+1}&pageSize=${pageView.pageSize }">下一页</a>
               </c:if>    
            </span></td>
		</tr>				
   </table>
